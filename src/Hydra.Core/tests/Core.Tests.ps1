...
Describe 'Global Setup' {
    ...
    It "Initializes paths correctly" {
        $script:CorePath | Should Not BeNullOrEmpty
    }
}
...
Describe 'AIConfig Module' -Tag 'Config' {
    ...
    Context 'Get-AIConfig' {
        ...
        It 'Should contain providers key' {
            $config = Get-AIConfig
            $config.ContainsKey('providers') | Should Be $true
        }

        It 'Should contain settings key' {
            $config = Get-AIConfig
            $config.ContainsKey('settings') | Should Be $true
        }

        It 'Should contain fallbackChain key' {
            $config = Get-AIConfig
            $config.ContainsKey('fallbackChain') | Should Be $true
        }

        It 'Should contain providerFallbackOrder key' {
            $config = Get-AIConfig
            $config.ContainsKey('providerFallbackOrder') | Should Be $true
        }
    }
    ...
    Context 'Get-DefaultConfig' {
        ...
        It 'Should have providers key' {
            $defaults = Get-DefaultConfig
            $defaults.ContainsKey('providers') | Should Be $true
        }

        It 'Should have fallbackChain key' {
            $defaults = Get-DefaultConfig
            $defaults.ContainsKey('fallbackChain') | Should Be $true
        }

        It 'Should have providerFallbackOrder key' {
            $defaults = Get-DefaultConfig
            $defaults.ContainsKey('providerFallbackOrder') | Should Be $true
        }

        It 'Should have settings key' {
            $defaults = Get-DefaultConfig
            $defaults.ContainsKey('settings') | Should Be $true
        }

        It 'Should have anthropic provider defined' {
            $defaults = Get-DefaultConfig
            $defaults.providers.ContainsKey('anthropic') | Should Be $true
        }

        It 'Should have openai provider defined' {
            $defaults = Get-DefaultConfig
            $defaults.providers.ContainsKey('openai') | Should Be $true
        }

        It 'Should have ollama provider defined' {
            $defaults = Get-DefaultConfig
            $defaults.providers.ContainsKey('ollama') | Should Be $true
        }

        It 'Should have maxRetries in settings' {
            $defaults = Get-DefaultConfig
            $defaults.settings.ContainsKey('maxRetries') | Should Be $true
        }

        It 'Should have rateLimitThreshold in settings' {
            $defaults = Get-DefaultConfig
            $defaults.settings.ContainsKey('rateLimitThreshold') | Should Be $true
        }

        It 'Should have autoFallback in settings' {
            $defaults = Get-DefaultConfig
            $defaults.settings.ContainsKey('autoFallback') | Should Be $true
        }
    }
    ...
    Context 'Test-ConfigValid' {
        ...
        It 'Should return true for valid config' {
            ...
            Test-ConfigValid -Config $validConfig | Should Be $true
        }
        ...
        It 'Should return false when providers key is missing' {
            ...
            Test-ConfigValid -Config $invalidConfig | Should Be $false
        }
...
Describe 'AIState Module' -Tag 'State' {
    ...
    BeforeAll {
        ...
        $stateFile = Join-Path (Split-Path $script:CorePath -Parent) "ai-state.json"
    ...
    AfterAll {
         # Clean up any state file created during tests
        $stateFile = Join-Path (Split-Path $script:CorePath -Parent) "ai-state.json"
        if (Test-Path $stateFile) {
            Remove-Item $stateFile -Force -ErrorAction SilentlyContinue
        }
    ...
    Context 'Get-AIState' {
        ...
        It 'Should have currentProvider property' {
            $state = Get-AIState
            $state.ContainsKey('currentProvider') | Should Be $true
        }

        It 'Should have currentModel property' {
            $state = Get-AIState
            $state.ContainsKey('currentModel') | Should Be $true
        }

        It 'Should have usage property' {
            $state = Get-AIState
            $state.ContainsKey('usage') | Should Be $true
        }

        It 'Should have errors property' {
            $state = Get-AIState
            $state.ContainsKey('errors') | Should Be $true
        }
    }
    ...
    Context 'Initialize-AIState' {
        ...
        It 'Should initialize usage for configured providers' {
            ...
            $state = Initialize-AIState -ConfigGetter $mockConfig
            $state.usage.ContainsKey('myProvider') | Should Be $true
        }
        ...
        It 'Should initialize usage tracking counters for models' {
            ...
            $state.usage['testProv']['model-x'].ContainsKey('totalTokens') | Should Be $true
            $state.usage['testProv']['model-x'].ContainsKey('totalRequests') | Should Be $true
        }
    }
    ...
    Context 'Update-AIState' {
        ...
        It 'Should create nested structure if provider does not exist' {
            ...
            $state.usage.ContainsKey('newProvider') | Should Be $true
            $state.usage['newProvider'].ContainsKey('newModel') | Should Be $true
        }
...
Describe 'AIConstants Module' -Tag 'Constants' {
    ...
    Context 'Thresholds Structure' {
        ...
        It 'Should have RateLimitWarning key' {
            $Thresholds.ContainsKey('RateLimitWarning') | Should Be $true
        }

        It 'Should have RateLimitCritical key' {
            $Thresholds.ContainsKey('RateLimitCritical') | Should Be $true
        }

        It 'Should have MaxRetries key' {
            $Thresholds.ContainsKey('MaxRetries') | Should Be $true
        }

        It 'Should have RetryDelayMs key' {
            $Thresholds.ContainsKey('RetryDelayMs') | Should Be $true
        }

        It 'Should have TimeoutMs key' {
            $Thresholds.ContainsKey('TimeoutMs') | Should Be $true
        }

        It 'Should have MaxConcurrent key' {
            $Thresholds.ContainsKey('MaxConcurrent') | Should Be $true
        }

        It 'Should have CpuLocalThreshold key' {
            $Thresholds.ContainsKey('CpuLocalThreshold') | Should Be $true
        }

        It 'Should have CpuHybridThreshold key' {
            $Thresholds.ContainsKey('CpuHybridThreshold') | Should Be $true
        }
    ...
    Context 'ProviderPriority Structure' {
        ...
        It 'Should be an array' {
            ($ProviderPriority -is [array]) | Should Be $true
        }

        It 'Should contain anthropic provider' {
            ($ProviderPriority -contains 'anthropic') | Should Be $true
        }

        It 'Should contain openai provider' {
            ($ProviderPriority -contains 'openai') | Should Be $true
        }

        It 'Should contain ollama provider' {
            ($ProviderPriority -contains 'ollama') | Should Be $true
        }
        ...
        It 'Should have at least 3 providers' {
            ($ProviderPriority.Count -ge 3) | Should Be $true
        }
    }
    ...
    Context 'TierScores Structure' {
        ...
        It 'Should have pro tier' {
            $TierScores.ContainsKey('pro') | Should Be $true
        }

        It 'Should have standard tier' {
            $TierScores.ContainsKey('standard') | Should Be $true
        }

        It 'Should have lite tier' {
            $TierScores.ContainsKey('lite') | Should Be $true
        }
    ...
    Context 'TaskTierMap Structure' {
        ...
        It 'Should have code task mapping' {
            $TaskTierMap.ContainsKey('code') | Should Be $true
        }

        It 'Should have simple task mapping' {
            $TaskTierMap.ContainsKey('simple') | Should Be $true
        }

        It 'Should have analysis task mapping' {
            $TaskTierMap.ContainsKey('analysis') | Should Be $true
        }

        It 'Should have general task mapping' {
            $TaskTierMap.ContainsKey('general') | Should Be $true
        }
    ...
    Context 'Paths Structure' {
        ...
        It 'Should have Config path' {
            $Paths.ContainsKey('Config') | Should Be $true
        }

        It 'Should have State path' {
            $Paths.ContainsKey('State') | Should Be $true
        }

        It 'Should have Cache path' {
            $Paths.ContainsKey('Cache') | Should Be $true
        }

        It 'Should have Modules path' {
            $Paths.ContainsKey('Modules') | Should Be $true
        }
    ...
    Context 'ErrorCodes Structure' {
        ...
        It 'Should have RateLimitExceeded code' {
            $ErrorCodes.ContainsKey('RateLimitExceeded') | Should Be $true
        }

        It 'Should have AuthenticationFailed code' {
            $ErrorCodes.ContainsKey('AuthenticationFailed') | Should Be $true
        }

        It 'Should have Timeout code' {
            $ErrorCodes.ContainsKey('Timeout') | Should Be $true
        }

        It 'Should have ProviderUnavailable code' {
            $ErrorCodes.ContainsKey('ProviderUnavailable') | Should Be $true
        }
    }
    ...
    Context 'ModelCapabilities Structure' {
        ...
        It 'Should have VisionModels array' {
            $ModelCapabilities.ContainsKey('VisionModels') | Should Be $true
            ($ModelCapabilities.VisionModels -is [array]) | Should Be $true
        }

        It 'Should have CodeModels array' {
            $ModelCapabilities.ContainsKey('CodeModels') | Should Be $true
            ($ModelCapabilities.CodeModels -is [array]) | Should Be $true
        }

        It 'Should have LongContextModels array' {
            $ModelCapabilities.ContainsKey('LongContextModels') | Should Be $true
            ($ModelCapabilities.LongContextModels -is [array]) | Should Be $true
        }

        It 'VisionModels should include Claude models' {
            ($ModelCapabilities.VisionModels -contains 'claude-opus-4-5-20251101') | Should Be $true
        }
